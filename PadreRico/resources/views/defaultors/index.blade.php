                ...
                <div class="col-xl-3 col-lg-4 col-md-6 d-flex">
                    <div class="card shadow-sm border-0 w-100 h-100 transition-all" style="border-radius: 1rem;">
                        <img src="{{ $debt->beneficiary->img ? asset('storage/' . $debt->beneficiary->img) : asset('imgs/9187604.png') }}"
                            class="card-img-top rounded-circle mx-auto mt-3" style="height: 120px; width: 120px; object-fit: cover; border-radius: 50%;"
                        ...
                        <div class="card-body d-flex flex-column text-center p-4">
                            ...
                            @if($debt->accepted == 0)
                                ...
                                <form action="{{ route('defaultors.accept', ['id' => Auth::id(), 'debt' => $debt->id]) }}" method="POST" class="d-flex justify-content-center gap-2 mb-2">
                                    @csrf
                            ...
                            <div class="d-flex justify-content-between mb-1">
                                <small class="text-muted">Desde: {{ \Carbon\Carbon::parse($debt->inicial_date)->format('d/m/Y') }}</small>
                                <small class="text-muted">Hasta: {{ \Carbon\Carbon::parse($debt->due_date)->format('d/m/Y') }}</small>
                            </div>
                        </div>
                    </div>
                </div>
